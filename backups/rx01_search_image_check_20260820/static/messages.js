...
let adminAccessPurpose = "messages";
...
function renderAccessRole() {
  ...
  if (adminAuthenticated) {
    ...
    $("accessRoleTitle").textContent = currentAdminName || "Modo administrador";
...
function syncAdminLoginButton() {
  ...
  $("adminLoginSubmit").disabled = !$("adminLoginName").value.trim() || !/^\d{5}$/.test($("adminPin").value);
}
...
function syncMessageComposerAccess() {
  ...
  $("messageAuthor").value = supervisorAuthenticated
    ...
    : adminAuthenticated ? (currentAdminName || "Administrador") : "";
...
$("adminLoginName").addEventListener("input", syncAdminLoginButton);
...
async function checkAdminSession() {
  ...
  const session = await response.json();
  adminAuthenticated = session.autenticado;
  currentAdminName = session.nombre || "";
...
function setAdminAccessPurpose(purpose = "messages") {
  adminAccessPurpose = purpose === "gps" ? "gps" : "messages";
  const gpsAccess = adminAccessPurpose === "gps";
  $("adminEyebrow").textContent = gpsAccess ? "UBICACIÓN PROTEGIDA" : "CONTROL DEL SISTEMA";
  $("adminTitle").textContent = gpsAccess ? "Acceso administrador para ver GPS" : "Panel del administrador";
  $("adminLoginSubmit").textContent = gpsAccess ? "Autorizar y ver GPS" : "Ingresar como administrador";
}

async function openAdmin(purpose = "messages") {
  setAdminAccessPurpose(typeof purpose === "string" ? purpose : "messages");
  $("adminModal").hidden = false;
  $("adminLoginError").textContent = "";
  $("adminStatus").textContent = "";
  await checkAdminSession();
  if (adminAuthenticated && adminAccessPurpose === "gps") {
    closeAdmin();
    window.dispatchEvent(new CustomEvent("mina:gps-access-granted"));
    return;
  }
  if (!adminAuthenticated) $("adminLoginName").focus();
}
window.openAdminForGps = () => openAdmin("gps");

function closeAdmin() { $("adminModal").hidden = true; }
$("adminOpen").addEventListener("click", () => openAdmin("messages"));
...
$("adminLoginForm").addEventListener("submit", async event => {
  ...
  setButtonLoading(submit, true, "Verificando…");
  let response;
  try { response = await fetch("/api/administrador/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ nombre: $("adminLoginName").value.trim(), pin: $("adminPin").value }) }); }
  ...
  currentAdminName = session.nombre || $("adminLoginName").value.trim();
  $("adminLoginForm").hidden = true;
  $("adminControls").hidden = adminAccessPurpose === "gps";
