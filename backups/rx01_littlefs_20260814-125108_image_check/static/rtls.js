...
function zoomMap(factor, clientX = null, clientY = null) {
  ...
  const nextWidth = Math.max(minimumMapViewWidth, Math.min(initialMapView.width, mapView.width * factor));
...
function rotateMap(direction) {
  ...
  else if (selectedMineLevel === "ALL") centerMapView(false);
...
function selectMapTag(svg, group, tagId) {
  ...
  if (marker && mapView.width > 150) {
    focusProjectedPoints([[Number(marker.getAttribute("cx")), Number(marker.getAttribute("cy"))]], 118);
  }
...
function renderMine() {
  ...
  for (const reader of mineLayout.readers || []) {
    ...
    const group = svgElement("g", {
      class: `reader-node ${reader.disponible ? "active" : "pending"}`,
      tabindex: "0",
      role: "button",
      "data-mine-area": `zone:${reader.id}`,
      "aria-label": `Centrar zona ${reader.id}`
    });
...
function focusMineArea(value) {
  if (!value) {
    ...
    if (hint) hint.textContent = "Selecciona una zona para centrarla automáticamente. Después puedes ampliar hasta 12× con el botón + o con dos dedos.";
    renderMine();
    if (selectedMineLevel === "ALL") centerMapView(true);
    else fitSelectedMineLevel();
    return;
  }
  const area = resolveMineArea(value);
...
function selectMineLevel(level) {
  ...
  const areaHint = $("mineAreaHint");
  if (areaHint) areaHint.textContent = "Selecciona una zona para centrarla automáticamente. Después puedes ampliar hasta 12× con el botón + o con dos dedos.";
  updateMineLevelButtons();
  renderMine();
  if (selectedMineLevel === "ALL") centerMapView(true);
  else fitSelectedMineLevel();
...
$("dxfFile").addEventListener("change", async event => {
  ...
  try {
    ...
    mineLayout = await response.json();
    renderMine();
    if (selectedMineLevel !== "ALL") fitSelectedMineLevel();
  } catch (error) {
...
$("resetView").addEventListener("click", () => {
  if (selectedMineArea) fitSelectedMineArea();
  else if (selectedMineLevel === "ALL") centerMapView(true);
  else fitSelectedMineLevel();
});
...
$("mineSvg").addEventListener("dblclick", event => {
  event.preventDefault();
  zoomMap(.52, event.clientX, event.clientY);
});
